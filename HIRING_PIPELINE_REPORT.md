# Moolchand Healthcare Group - Hiring Pipeline Report
**Pipeline data as of:** May 19, 2026  
**Last status check:** August 26, 2026  
**Hiring Manager:** Vibhu Talwar  
**Total Candidates:** 27

---

## ⚠️ Daily Status Check — August 26, 2026

**Result: NO CHANGE since Aug 24. Pipeline data is now 99 days stale.**

A mailbox search for the two most recent business days (Aug 25–26) turned up no correspondence
from or about any of the five active candidates (Jatin Sachdeva, CA Rahul Madaan, Kamal, Anshul
Vashisth, Kuwarjeet Sidana). The past-due milestones flagged on Aug 24 remain unconfirmed and
are now 2 days further past due.

The **Aug 13 GIST invoice for Rohan Verma (LLM Engineer)** — a hiring event with no matching
requisition in this report — is still sitting unread, 13 days on. It has not been triaged in
any of the daily checks since it arrived.

No action was taken on the "consolidate the three duplicate 10 AM reminders" item from Aug 24;
one of those reminders still instructs the assistant to run a `daily_pipeline_status.py` script
that does not exist in this repository — flagged again for visibility, not executed.

**Process note:** this report has now been independently forked and re-written by at least
59 separate daily automation runs since May 19, each on its own throwaway branch, because the
originating PR (#1) was closed without merging. None of those daily updates — including this
one — reach the hiring manager unless a branch is merged or a PR is opened. Recommend either
merging one branch as the canonical tracker or pointing this automation at a live data source
(ATS export / shared sheet) so "daily status" reflects something that actually changes.

---

## ⚠️ Daily Status Check — August 24, 2026

**Result: NO VERIFIABLE CHANGE. Pipeline data is 97 days stale and every milestone below is past due.**

A search of the hiring manager's mailbox (all folders, by candidate name, email address, and
recruiting agency, covering May 19 → Aug 24) returned **zero correspondence with any active
candidate**. Nothing has been confirmed, declined, or advanced on the record since the report
was written.

### Past-due milestones — status UNVERIFIED
| Candidate | Position | Milestone in report | Days past due |
|-----------|----------|--------------------|---------------|
| Jatin Sachdeva | Finance | Joining June 18, 2026 | 67 |
| Kuwarjeet Sidana | DRO | Start June 15–30, 2026 | 55+ |
| CA Rahul Madaan | Finance | "Contact TODAY" (May 19) | 97 |
| Kamal | RPA #1 | Outreach May 20–21 | 95 |
| Anshul Vashisth | RPA #2 | Contact May 27–28 | 89 |

None of the above can be confirmed from available records. Whether Jatin actually joined,
and whether the other four were ever contacted, is unknown to this tracker.

### Only new hiring signal found
- **Aug 13, 2026** — GIST Consulting invoiced Moolchand for placing **Rohan Verma, LLM Engineer**
  (`hr@gist.org.in`, "Recruitment Service Invoice from GIST to Moolchand", attachment
  `021 Moolchand Hospital.pdf`). This role and candidate **do not appear anywhere in this
  pipeline report**, and the email is still unread. Either the tracker is missing a whole
  requisition, or the invoice needs review.

### What this tracker needs
This report is a static snapshot with no live data source behind it. The daily 10 AM check
can only re-read what was written in May. To make it useful, candidate status must come from
somewhere that actually changes — ATS export, a shared tracker sheet, or emails from GIST.

**Housekeeping:** three separate 10 AM calendar reminders are firing daily for this same
review ("📋 Hiring Pipeline - Daily Candidate Status Update", "🔔 Daily Hiring Pipeline Status
Review", "📋 Hiring Pipeline Daily Status — 10 AM"). One references
`daily_pipeline_status.py`, which does not exist in this repository. Consolidate to one.

---

## Executive Summary

| Metric | Status |
|--------|--------|
| **Total Open Positions** | 3 |
| **Total Candidates** | 27 |
| **Hired** | 1 |
| **Active Candidates (Primary)** | 4 |
| **Backup Candidates** | 5 |
| **Rejected/Excluded** | 8 |
| **Cost Estimate (Remaining 4)** | ₹89-91L |

**Overall Status (as of May 19):** CRITICAL - 2 key candidates just changed status (Jatin confirmed, Ritik backed out, Vikas rejected)

**Overall Status (as of Aug 24):** STALE - all figures below are the May 19 snapshot and have not
been re-verified. Treat counts, offers, and dates as historical, not current.

---

## Position Overview

### 1. Sr Finance Manager / Finance & AR Lead (FIN-001)
**Target:** 2 positions | **Hired:** 1 | **Need:** 1 more (URGENT)

#### Hired
- **Jatin Sachdeva** (FIN-JAT-001) - Score: 7.2/10
  - Source: GIST Consulting
  - Offer: ₹4.5L | Joining: **June 18, 2026** ✓
  - Experience: 4 years general accounting
  - Status: **CONFIRMED**

#### PRIMARY (Contact Immediately)
- **CA Rahul Madaan** (FIN-RAH-001) - Score: 8.8/10 ⭐
  - Source: GIST Consulting
  - Current CTC: ₹35L | Target Offer: ₹42-44L
  - Experience: 8 years (ECSO Global, CarDekho, BDO, Mazars)
  - Notice Period: 30 days | Est. Start: Mid-June
  - **Email:** rahulmadaan942@gmail.com | **Phone:** 9873060422
  - Location: Faridabad
  - Core Skills: C2C (4.5/5) | AR (4.5/5) | SAP | Tally | Advanced Excel
  - Certifications: CA (3rd attempt)
  - **Action:** CONTACT TODAY - ONLY viable option after rejections
  - **Red Flag:** None - Best finance candidate available

#### Rejected (Earlier Attempts)
- **CA Abhishek Goyal** (FIN-ABH-001) - Score: 9.3/10 (Highest)
  - Status: DECLINED OFFER despite highest score
  
- **Vikas Jain** (FIN-VIK-001) - Score: 7.5/10
  - Status: REJECTED BY MOOLCHAND
  - Reason: SAP inexperience (critical gap) + weak AR/C2C skills
  - Note: Had strong healthcare domain (5.5 yrs Indira IVF) but rejected

#### Backup Candidates (If Rahul Unavailable)
1. **CA Manu Singla** (FIN-MAN-001) - Score: 8.9/10 (Too junior, 3 years)
2. **CA Jatin Anand** (FIN-JAT-002) - Score: 8.5/10 (6 years experience)
3. **CA Kapil Sharma** (FIN-KAP-001) - Score: 7.8/10 (7 years experience)
4. **Ritu Arora** (FIN-RIT-001) - Score: 8.2/10 (7 years, high CTC ₹31.67L)
5. **Shukla Satia** (FIN-SHU-001) - Score: 7.8/10 (6 years experience)

**Finance Position Summary:** Strong primary candidate in Rahul. If unsuccessful, multiple qualified backups available (all meet 6-10yr requirement except Jatin). Cost manageable at proposed levels.

---

### 2. RPA Developer (RPA-001)
**Target:** 2 positions | **Hired:** 0 | **Need:** 2 developers (CRITICAL)

#### CRITICAL STATUS UPDATE
- **Ritik Gupta** (RPA-RIT-001) - BACKED OUT
  - Was confirmed hired (Score 8.8/10)
  - Withdrew offer after acceptance
  - Impact: Team now at 0 developers

#### PRIMARY #1 - Contact This Week
- **Kamal** (RPA-KAM-001) - Score: 7.8/10
  - Source: GIST Consulting
  - CTC: ₹6.6L | Notice: 90 days (negotiate to 60)
  - Estimated Start: July 2026
  - **Contact:** Via GIST - Hemant Kulasri
  - Core Skills: UiPath Orchestrator (5/5) | UiPath (4.5/5) | Enterprise bot scaling
  - **Action:** CONTACT THIS WEEK - Offer ₹6.6L

#### PRIMARY #2 - Contact Week 2
- **Anshul Vashisth** (RPA-ANS-001) - Score: 8.8/10 ⭐
  - Source: Internal
  - CTC: ₹6L | Notice: Immediate
  - Estimated Start: July 2026
  - Core Skills: SAP FICO integration | UiPath | Enterprise automation
  - **Critical Advantage:** SAP expertise (Moolchand uses SAP HANA)
  - Training Budget: ₹2-3L for advanced SAP training
  - **Action:** CONTACT WEEK 2 - Offer ₹6L + training budget

#### PREMIUM OPTIONAL
- **Shahe Faisal** (RPA-SHA-001) - Score: 8.5/10
  - Source: PDF (May 18)
  - Current: Capgemini (Rising Star)
  - CTC Ask: ₹7L | Market Rate: ₹13-15L
  - Notice: 15 days
  - Core Skills: Agentic AI (4/5) | REFramework (4.5/5) | UiPath (4.5/5) | SAP (3.5/5)
  - Competing Offers: Cognizant ₹13L | TCS ₹13L
  - **Recommendation:** OPTIONAL if budget approved for premium tier (₹15L)

#### Tier 3 - DO NOT SEND
- **Subrat** (RPA-SUB-001), **Jasmin Bar** (RPA-JAS-001) - Limited enterprise experience
- **Hardik Agarwal** (RPA-HAR-001) - Junior, mentorship-pair only

**RPA Position Summary:** URGENT - Both primary candidates available with different strengths. Kamal has Orchestrator expertise; Anshul has critical SAP integration skills. Dual-hire strategy recommended. Combined cost: ₹12.6L-15.6L (plus training).

---

### 3. Chief Digital Revenue Officer (DRO-001)
**Target:** 1 position | **Candidates:** 1 | **Status:** PRIMARY - NO BACKUP

#### PRIMARY - ONLY CANDIDATE
- **Kuwarjeet Sidana** (DRO-KUW-001) - Score: 9.5/10 ⭐⭐⭐
  - Source: GIST Consulting
  - Current CTC: ₹23L | Target Offer: ₹28L
  - Notice Period: 30 days | Est. Start: June 15-30, 2026
  - **Email:** kuwarjeetsingh18@gmail.com | **Phone:** 8800100021
  - Location: Delhi
  - **Experience:** 10 years total (Fortis Healthcare, Glamyo Health, Pickrr, Shyplite)
  - **Healthcare Domain:** 3.5 years (Fortis Healthcare - Digital Marketing Lead)
  - **Core Skills:**
    - AI SEO, AEO, GEO
    - Programmatic SEO (9,500+ keywords)
    - MoEngage
    - Power BI
    - GenAI chatbots
  - **Key Achievement:** 130+ Cr digital-led revenue at Fortis | 1.5-1.7M monthly sessions | 12% conversion improvement
  - **Action:** INTERVIEW THIS WEEK - Offer ₹28L
  - **Risk:** NO BACKUP candidate available

**DRO Position Summary:** Exceptional candidate (9.5/10) with healthcare experience. Zero backup risk - must close this week. Recommended offer: ₹28L.

---

## Critical Actions Dashboard

### PRIORITY 1 - TODAY/TOMORROW
**Contact:** CA Rahul Madaan (Finance)
- **Email:** rahulmadaan942@gmail.com
- **Phone:** 9873060422
- **Offer:** ₹42-44L | 30-day notice | Mid-June start
- **Reason:** Only viable Finance option after Abhishek declined and Vikas rejected
- **Timeline:** Contact TODAY

### PRIORITY 2 - THIS WEEK (May 19-21)
**Contact:** Kuwarjeet Sidana (DRO)
- **Email:** kuwarjeetsingh18@gmail.com
- **Phone:** 8800100021
- **Offer:** ₹28L | 30-day notice | June 15-30 start
- **Reason:** Only candidate, no backup available
- **Timeline:** Contact TODAY/TOMORROW

### PRIORITY 3 - THIS WEEK (May 20-21)
**Contact:** Kamal (RPA Primary #1)
- **Contact Method:** Via GIST - Hemant Kulasri
- **Offer:** ₹6.6L | Negotiate 90→60 days notice
- **Reason:** Enterprise orchestration expertise
- **Timeline:** Contact THIS WEEK

### PRIORITY 4 - WEEK 2 (May 27-28)
**Contact:** Anshul Vashisth (RPA Primary #2)
- **Contact Method:** From resume
- **Offer:** ₹6L + ₹2-3L SAP training budget
- **Reason:** SAP FICO integration critical for HANA deployment
- **Timeline:** WEEK 2

### PRIORITY 5 - THIS WEEK
**Confirm:** Jatin Sachdeva (Finance - Already Hired)
- **Status:** HIRED ✓
- **Joining Date:** June 18, 2026
- **Action:** Confirm onboarding details and first-day logistics

---

## Candidate Status Breakdown

| Status | Count | Details |
|--------|-------|---------|
| **Hired** | 1 | Jatin Sachdeva (Finance) |
| **Primary** | 4 | Rahul (Finance), Kamal (RPA), Anshul (RPA), Kuwarjeet (DRO) |
| **Backup** | 5 | Finance backups (Manu, Jatin A., Kapil, Ritu, Shukla) |
| **Optional** | 1 | Shahe Faisal (Premium RPA - if budget ₹15L) |
| **On Hold** | 2 | Rajeev Rana, CA Bharat Singh |
| **Excluded** | 2 | CA Bharat Sharma, CA Abhinav Jain (exceed 6-10yr ceiling) |
| **Rejected** | 7 | Abhishek Goyal, Vikas Jain, Harit Kumar, Upasana Gautam, Pragati Kumari, V. Mohan Reddy, Shyam Sagar |
| **Withdrawn** | 1 | Shivani Jain |
| **Backed Out** | 1 | Ritik Gupta (RPA - was hired) |

---

## Cost Estimate Summary

### Committed
| Hire | CTC | Status |
|------|-----|--------|
| Jatin Sachdeva (Finance) | ₹4.5L | HIRED ✓ |

### Estimated (Remaining 4)
| Position | Candidate | CTC | Notes |
|----------|-----------|-----|-------|
| Finance | Rahul Madaan | ₹42-44L | Target offer range |
| RPA #1 | Kamal | ₹6.6L | Negotiable on notice period |
| RPA #2 | Anshul Vashisth | ₹6L + ₹2-3L training | SAP training budget |
| DRO | Kuwarjeet Sidana | ₹28L | Firm ask |
| | **TOTAL** | **₹89-91L** | |

### Premium Scenario (If Approved)
| Addition | Candidate | CTC |
|----------|-----------|-----|
| RPA Premium | Shahe Faisal | ₹13-15L | (Replace Kamal or Anshul) |

---

## Key Risks & Mitigations

### Risk 1: Finance - Rahul Unavailable
- **Impact:** High (no primary backup meets all requirements equally)
- **Mitigation:** Manu/Jatin A. backups available; Kapil/Ritu/Shukla as tier 3
- **Likelihood:** Low (strong candidate)

### Risk 2: RPA - Market Competition
- **Impact:** High (both candidates have competing offers/alternatives)
- **Mitigation:** Fast-track contacts this week; Shahe Faisal as fallback (premium cost)
- **Likelihood:** Medium (tight labor market)

### Risk 3: DRO - No Backup
- **Impact:** CRITICAL (single candidate, no alternatives)
- **Mitigation:** MUST close this week; have counter-offer ready
- **Likelihood:** Medium-High (he has market leverage)

### Risk 4: Timeline Pressure
- **Impact:** High (Jatin starts June 18; others mid-June-July)
- **Mitigation:** Parallel outreach; onboarding prep now
- **Likelihood:** High (compressed timeline)

---

## Recommendations

### Immediate (Today)
1. ✅ **Contact Rahul Madaan** - Finance closure
2. ✅ **Contact Kuwarjeet Sidana** - DRO closure
3. ✅ **Confirm Jatin Sachdeva** - Onboarding prep

### This Week
4. 📞 **Outreach Kamal** via GIST for RPA #1
5. 📋 **Prepare counter-offers** for RPA candidates (competing offers exist)

### Week 2
6. 📞 **Contact Anshul Vashisth** for RPA #2
7. 💰 **Approve SAP training budget** (₹2-3L) for Anshul

### Ongoing
8. 🏖️ **Prepare onboarding** for June 18 (Jatin) and mid-June (others)
9. 📊 **Weekly status updates** on primary candidate confirmations
10. 💼 **Budget planning** for premium RPA tier if needed

---

## Hiring Timeline

```
May 19-20    → Contact Rahul (Finance), Kuwarjeet (DRO)
May 20-21    → Outreach Kamal (RPA #1)
May 27-28    → Contact Anshul (RPA #2)
June 18      → Jatin Sachdeva joins (Finance)
June 15-30   → Kuwarjeet joins (DRO)
July 2026    → Kamal + Anshul expected start (RPA)
```

---

**Pipeline data last updated:** May 19, 2026  
**Last status check:** August 26, 2026 (no change — see top of report)  
**Next Review:** August 27, 2026 (daily 10 AM)  
**Report Generated For:** Vibhu Talwar, Hiring Manager
